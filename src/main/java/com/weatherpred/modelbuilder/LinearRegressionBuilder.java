...
/**
 * Class to build a Linear Regression Model with required algorithmic parameters
 * 
 * Date : August 2, 2017
 * 
 * @author Poornima Tom
...
public class LinearRegressionBuilder {
	...
	private final static Logger logger = LoggerFactory
			.getLogger(LinearRegressionBuilder.class);

	/**
	 * Linear Regression model to predict Temperature
	 */
	private static LinearRegressionMlModel temperatureModel;
	/**
	 * Linear Regression model to predict Humidity
	 */
	private static LinearRegressionMlModel humidityModel;
	/**
	 * Linear Regression model to predict Pressure
	 */
	private static LinearRegressionMlModel pressureModel;

	/**
	 * The static block populates the model with required ML parameters
	 */
	static {
		temperatureModel = MLModelUtil.populateModelParams(
	...
	public static void main(String[] args) {
		SparkConf sparkConf = new SparkConf().setAppName(
				Constants.LINEAR_REGRESSION_BUILDER_APP_NAME).setMaster(
	...
	private static JavaRDD<LabeledPoint>[] getTrainingAndTestSplit(
		...
		try {
			...
			case TEMPERATURE:
				...
				List<LabeledPoint> l = labelledPointForTemperature.collect();
